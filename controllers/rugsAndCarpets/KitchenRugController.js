const KitchenRug = require('../../models/RugsAndCarpets/KitchenRug');

// Create a new rug
const createRug = async (req, res) => {
    try {
        const newRug = new KitchenRug({
            ...req.body,
        ...
        res.status(201).json({
            success: true,
            message: 'Kitchen Rug created successfully',
            rug: newRug
        });
    } catch (error) {
...
const getAllRugs = async (req, res) => {
    try {
        const rugs = await KitchenRug.find();
        res.status(200).json({
            success: true,
...
const getRugById = async (req, res) => {
    try {
        const rug = await KitchenRug.findById(req.params.id);
        if (!rug) {
...
const deleteRug = async (req, res) => {
    try {
        const rug = await KitchenRug.findByIdAndDelete(req.params.id);
        if (!rug) {
            return res.status(404).json({
                success: false,
                message: 'Rug not found'
